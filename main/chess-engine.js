...
class ChessEngine {
    ...
    getPieceAt(position) {
        ...
        const piece = this.pieces.find(piece => piece.position.row === position.row &&
            piece.position.col === position.col) || null;
        // 缓存结果
        if (this.cache) {
    ...
    getPiecesByType(type, color) {
        return this.pieces.filter(piece => piece.type === type && piece.color === color);
    }
    /**
     * 移动棋子
    ...
    isValidMove(piece, toPosition) {
        ...
        if (piece.color !== this.currentPlayer)
            return false;
        // 位置验证
        if (toPosition.row < 0 || toPosition.row > 9)
            return false;
        if (toPosition.col < 0 || toPosition.col > 8)
            return false;
        // 不能吃自己的棋子
        const targetPiece = this.getPieceAt(toPosition);
    ...
    validateElephantMove(piece, toPosition) {
        ...
        if (rowDiff !== 2 || colDiff !== 2)
            return false;
        // 象不能过河
        if (piece.color === 'red' && toPosition.row < 5)
            return false;
        if (piece.color === 'black' && toPosition.row > 4)
            return false;
        // 检查象眼是否被堵
        ...
        if (this.getPieceAt(elephantEye))
            return false;
        return true;
    }
    /**
    ...
    validateRookMove(piece, toPosition) {
        const { position: from } = piece;
        // 车只能走直线
        if (from.row !== toPosition.row && from.col !== toPosition.col)
            return false;
        // 检查路径是否有障碍
        if (from.row === toPosition.row) {
            ...
            for (let col = minCol + 1; col < maxCol; col++) {
                if (this.getPieceAt({ row: from.row, col }))
                    return false;
            }
        }
        else {
            ...
            for (let row = minRow + 1; row < maxRow; row++) {
                if (this.getPieceAt({ row, col: from.col }))
                    return false;
            }
        }
        return true;
    ...
    areKingsFacing() {
        ...
        if (!redKing || !blackKing)
            return false;
        if (redKing.position.col !== blackKing.position.col)
            return false;
        // 检查两将之间是否有棋子
        ...
        for (let row = minRow + 1; row < maxRow; row++) {
            if (this.getPieceAt({ row, col: redKing.position.col }))
                return false;
        }
        return true;
    }
